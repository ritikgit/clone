...
class MirrorListener(listeners.MirrorListeners):
	...
	def onUploadComplete(self, link: str):
		with download_dict_lock:
			...
			self.buildbutton("⚡Drive Link⚡", link)
			LOGGER.info(f'Done Uploading {download_dict[self.uid].name()}')
			if INDEX_URL is not None:
				share_url = requests.utils.requote_uri(f'{INDEX_URL}/{download_dict[self.uid].name()}')
				if os.path.isdir(f'{DOWNLOAD_DIR}/{self.uid}/{download_dict[self.uid].name()}'):
					share_url += '/'
				self.buildbutton("Index Link", share_url)
			...
			del download_dict[self.uid]
			count = len(download_dict)
		sendMarkup(msg, self.bot, self.update, InlineKeyboardMarkup(self.button))
		if count == 0:
			self.clean()
		else:
			update_all_messages()
	...
	def buildbutton(self, key, link):
		self.button.append([InlineKeyboardButton(text = key, url = link),
							InlineKeyboardButton("🥳TeamDrive🥳", "http://drive.google.com/drive/u/2/mobile/folders/0AE-Gnz5WSzofUk9PVA/15nHk2eTQxNL3XyEi8r7-2QtPIB40u2C0")], 
							[InlineKeyboardButton("Group", "http://groups.google.com/forum/#!forum/teamdrivee/join")])
